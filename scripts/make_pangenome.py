...
def write_output(output_folder, name_log, contribution_contigs, contribution_nb_bp, keys): 
    with open(output_folder + name_log + ".log", "w") as fillresult :
...
def gestion_fragment(ref_in,potentiel_inser) :
    ...
    if p :
        ...
        #seq_to_change=str(dict_ref[name_contig].seq)
        #list_position=list(dico_bornes.keys())
...
def make_reference(output_tmp, nb_contigs, len_hit_mini, IDENTITY, output_folder, ref_name, name_log):         
    ...
    for record in SeqIO.parse(output_tmp, "fasta"):
        ...
        SeqIO.write(record,"tmp.fasta","fasta-2line")
        
        blastn_cline = NcbiblastnCommandline(query="tmp.fasta", subject=ref_in, out="test_sortie.xml",perc_identity=IDENTITY, outfmt=5)

        blastn_cline() #lancer un blast
        print("record numéro :" + record.id)
        #print(timetime)
        result_handle = open("test_sortie.xml")
        blast_record = NCBIXML.read(result_handle)
        q_dict = SeqIO.index("tmp.fasta", "fasta") # contient le contig entier
        ...
        if blast_record.alignments :
            ...
            for seq in orphan_records : 
                ...
                if len(seq) > len_hit_mini : # vérification de la taille des contigs qui n'ont pas hit
                    ...
                    nb_bp=nb_bp+len(seq)
            SeqIO.write(orphan_length_ok,"record_mauvais","fasta")
        ...
        #pas de hit, donc ajout du contig entier    
            ...
            subprocess.call("cat " + "tmp.fasta" + " >> " + ref_in, shell=True)  
...
if __name__ == "__main__":            
    ...
    if os.path.isfile(args.file_in) == True :
        paths=read_path_file(args.file_in, args.default_path)
        dict_name_seq, list_name, list_length = read_contigs(paths, args.nb_contigs, args.hit_min)
    
    elif os.path.isdir(args.file_in) :
        dict_name_seq, list_name, list_length = read_contigs(glob.glob(args.file_in +"*_contigs.fasta"),args.nb_contigs, args.hit_min)
    else :
